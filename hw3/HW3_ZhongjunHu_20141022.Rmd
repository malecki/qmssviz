...
dp <- data.table(df)
dp <- dp[, list(ie = unique (ie), percentage = as.numeric(table(ie)/length(ie), by=c("year", "Country"))), by=c("year", "Country")] #Get percentages#
dp
...
```
      ...
      2. I think there are some bugs in my second and third graphs, but I cannot find out what goes wrong. In the second graph, the distribution doesn't match with the result in the first and third graph very well. I think the order of labels is not consistent. In the third graph, the guide in the right is strange that "10" ranks in the second order. Anyone could help me?
