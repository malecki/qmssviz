...
I am interested in the issues of income distribution in Asia in the past 20 years. China, Japan, and South Korea are top 3 properous countries in east Asia. So I choose the 3 countries to analyze what I am interested in. 

Two waves from World Values Survey are selected, wave 6 (2010-2014) and wave 2 (1990-1994). I intend to find out what changes of people's feelings about social equality have happenned in the 20 years.
...
I want to plot two pictures.

The first is a segment-point plot. X-axis is "country" and Y-axis is the mean of each country. There are two segments above each country to indicate the mean value in different waves, namely the year of 1990 and 2000. This one is designed to demonstrate the comparison of average level of 2 waves in each country. 

Another one is a bar graph. X-axis is "answer scale"", which contains "1" to "10". Y-axis is "percentage", which means the percentage of prequency of an answer (like "5") in the total sum of answers in a country in one wave. Above the X-axis is the "percentage" in the year of 1990, while beneath the X-axis is "percentage" in the year of 2012.

#####(I tried to import my sketch but failed.
...
df$ie[df$iechr=="Individ"]<- "Income differences are incentives"
df$ie[df$iechr=="Equal"]<- "Incomes should be equal"
df$ie[df$iechr=="Incomes should be made more equal"]<- "Incomes should be equal"
df$ie[df$iechr=="We need larger income differences as incentives for individu"]<- "Income differences are incentives"
df$ie[df$iechr=="2"]<- "Incomes should be equal"
df$ie[df$iechr=="3"]<- "Incomes perhaps could be more equal"
df$ie[df$iechr=="4"]<- "Incomes perhaps could be more equal"
df$ie[df$iechr=="5"]<- "Totally neutral about that"
df$ie[df$iechr=="6"]<- "Totally neutral about that"
df$ie[df$iechr=="7"]<- "Income differences may be incentives"
df$ie[df$iechr=="8"]<- "Income differences may be incentives"
df$ie[df$iechr=="9"]<- "Income differences are incentives"
...
d <- data.table(df)
d <- d[, list(ie = unique(ie), percentage = as.numeric(table(ie)/length(ie))), by = c("year", "Country")] #Get percentages#
```
...
ggplot(d, aes(x=Country, y=percentage, fill=ie))+
