...
```{r}
dd <- read.csv("~/Desktop/qmssviz/Lab1/Donation-Disease.csv")
```
...
```{r, echo=TRUE}
money<-dd[ ,3]
money_range <- range (0, money) #the range depends on what the range of money is
plot(money, type="o", col="blue", axes=FALSE, xlab="Disease", ylab="Money Raised (in millions)") #we let axes=FALSE because we want to name the axes ourselves using the two axis commands below
axis(1, at=1:8, lab=c("heartd", "suicide", "breastc", "diabetes", "HIV", "prostatec", "motord", "chronicd"))
axis(2, las=1, at=50*0:money_range[2])#las=1 means that we want the numbers on the y-axis to be horizontally labeled. The at command means that we want the tick-marks to be drawn every 50 units
```
...
```{r}
JFK <-read.csv("~/Desktop/JFK_snow.csv")
```
