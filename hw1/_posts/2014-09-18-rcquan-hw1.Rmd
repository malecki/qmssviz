...
First, let's load the relevant libraries into the R environment and set the current working directory.
...
Next, let's define some functions to help us get and process the data more easily. `HTMLtoDF` parses the HTML tree, retreives values enclosed in the `</td>` tag, and plops them into a `data.frame`. `RemoveSpaces` uses a regular expression to remove leading and lagging spaces.
...
## Total Meetings Held by Committee by Year
...
* The `Committee on Finance` has consistently held far more meetings than any other committee since the year 2000
* Some committees have held far less meetings than others

This `ggvis` plot allows the user to compare the number of committee meetings held by each committee for each month. However, the use of total number should be viewed with skepticism since not all committees were active since the year 2000. This will be addressed in future version of this graph. 

Also, the recyling of color palette does not make multiple comparisons very easy for the user to interpret. This can be addressed by categorizing the committees into more meaningful groups.
