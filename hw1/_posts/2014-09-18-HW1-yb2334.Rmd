...
```{r}
plot1 <- qplot(Program, data = data, color = Program, size = I(1.5), main = "Student Background Summary")
last_plot() + theme(axis.text.x = element_blank()) + coord_fixed(ratio = 0.3)
```

2.2 Baseline Experience Analysis

The second plot demonstrates top 10 most popular computer tools among Data Visualization students. (has problem)
```{r}
sub_tool <- tools[1:10, ]
plot2 <- qplot(name, count, data = sub_tool, main = "Top 10 Most Popular Computer Tools among Data Visualization Students", goem = "bar")
...
```{r}
plot4 <- qplot(data[, 6], data = data, fill = Program)
```