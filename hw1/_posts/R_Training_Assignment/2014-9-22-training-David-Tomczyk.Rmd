...
full_time.total <- aggregate(full_time ~ state,data=southern_pop3,sum)
state_pop.total <- aggregate(pop_2006 ~ state,data=southern_pop3,sum)

final <- merge(full_time.total, state_pop.total, by = "state")

final.sorted <- final[order(final$pop_2006, decreasing=TRUE), ]
