...
progtwit2 <- data.frame(revalue(progtwit$Program, c("QMSS  First semester" = "QMSS 1st Sem.")))
#progtwit2 <- data.frame(revalue(progtwit$Program, c("QMSS Second semester +" = "QMSS 2nd Sem. +")))
#progtwit2 <- data.frame(revalue(progtwit$Program, c("QMSS Third semester" = "QMSS 3rd Sem.")))
...
```{r ggplot2, fig.width=18, fig.height=8, message=FALSE}

twit.title <- ggtitle("Twitter Users by Program")
dotplot <- geom_dotplot(stackgroups=TRUE, binpositions="all")


ggplot(progtwit, aes(Program, fill=Do.you.use.twitter.))+ geom_bar() + twit.title + x.axis + y.axis

ggplot(progtwit, aes(Program, fill=Do.you.use.twitter.))+ dotplot + twit.title + x.axis + y.axis
