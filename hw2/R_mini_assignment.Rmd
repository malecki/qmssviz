...
```{r import-data}
setwd("~/github/qmssviz-FORK/labs/tutorial/") # set to directory containing data files
lgas<-read.csv("lgas.csv")
str(lgas)
dim(lgas)

facilities<-read.csv("sample_health_facilities.csv")
str(facilities)
dim(facilities)
```
...
But wait... are our population totals correct? Are all LGA's represented in the `sample_health_facilities` data set such that aggregating population bottom-up would give an accurate state total?
```{r pop}
# lgas in the "South" of Nigeria
row<-grep("South", lgas$zone)
SouthLgas<-lgas[row, ]
SouthLgas<-droplevels(SouthLgas)
check<-levels(SouthLgas$lga)
compare<-levels(df$lga)
check %in% compare # are all unique LGAs in a Southern Zone contained in the sample_health_facilies subset?  NO!
```