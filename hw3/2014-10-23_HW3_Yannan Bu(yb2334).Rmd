...
I plan to plot two maps showing the mean value of respondents' answers to this question of each country, and another bar plot demonstrating the difference between two waves. I expect that there is a contrast between the West and the East, yet this difference will shrink in Wave 5. 
...
map1 <- mapCountryData(plot1, nameColumnToPlot = "MarrOut", colourPalette = color, addLegend = F, mapTitle = "Marriage is an out-dated institution? (Wave 3) 1 - agree, 2 - disagree")
...
par(mai=c(0,0,0.2,0),xaxs="i",yaxs="i")
map2 <- mapCountryData(plot2, nameColumnToPlot = "MarrOut", colourPalette = color, addLegend = F, mapTitle = "Marriage is an out-dated institution? (Wave 5) 1 - agree, 2 - disagree")
do.call(addMapLegend
        , c(map2
            ,legendWidth = 0.4))
```
...
plot3 <- ggplot(data_all, aes(x = Country, y = value, group = Wave, fill = Wave)) +
  ...
  scale_y_discrete(limits = c(1, 2), label = c("Agree", "Disagree")) +
  xlab(" ") +
  ylab(" ") +
  ggtitle("Marriage is an out-dated institution?") +
...
selectVariables <- function(Survey, variable) {
  subset(Survey, select = c(V2, variable), variable > 0)
}
...
plotData <- function(Survey) {
  ...
  par(mai=c(0,0,0.2,0),xaxs="i",yaxs="i")
  map <- mapCountryData(plot, nameColumnToPlot = "MarrOut", colourPalette = color, addLegend = F, mapTitle = "Marriage is an out-dated institution? 1 - agree, 2 - disagree")
do.call(addMapLegend
        , c(map1
            ,legendWidth = 0.4))
  return(map)
